...
//   const map = new Map();            // 1. Создаем пустую «таблицу» для хранения найденных чисел и их индексов
//   for (let i = 0; i < nums.length; i++) {  // 2. Проходимся по всем элементам массива
//     let complement = target - nums[i];      // 3. Считаем дополняющее число: что надо добавить к nums[i], чтобы получить target
    
//     if (map.has(complement)) {              // 4. Проверяем, есть ли это дополняющее число, которое мы нашли в прошлом
//       return [map.get(complement), i];     // 5. Если есть — возвращаем пару индексов (индекс complement и индекс текущего числа)
//     }
    
//     map.set(nums[i], i);                    // 6. Если дополняющего числа нет, запоминаем текущее число и его индекс (на будущее)
//   }
//   return null;                             // 7. Если не нашли пары, возвращаем null
// };



// function isValid(s) {
//   // Объект, сопоставляющий закрывающие скобки с соответствующими им открывающими
//   const pairs = {
//     ')': '(',
//     ']': '[',
//     '}': '{'
...
//   // Идём по каждому символу входной строки
//   for (let char of s) {
...
//       // Проверяем, пуст ли стек. Если пуст, значит нет открывающей, которая её могла бы закрыть — строка невалидна
//       if (stack.length === 0) return false;

//       // Достаём из стека последнюю открывающую скобку
//       const top = stack.pop();

//       // Проверяем, соответствует ли она текущей закрывающей с помощью объекта pairs
//       if (pairs[char] !== top) {
//         // Если не соответствует — строка невалидна
//         return false;
//       }
//     }
//   }

//   // После прохода по всей строке, если стек пуст — значит все скобки корректно закрылись
//   // Если стек не пуст — остались незакрытые скобки — строка невалидна
